...
def _source_editor() -> None:
    ...
    st.caption(
        ...
        "Мощность ограничивает физический объём поставки; цена влияет на закупочные расходы; "
        "резервирование — на стоимость закреплённой мощности; take-or-pay — на минимально оплачиваемый объём; "
        "срок поставки сдвигает момент прибытия топлива."
    )
...
def _scenario_editor() -> None:
    ...
    st.caption(
        "Множитель 1,00 оставляет параметр без изменений; 1,10 означает +10%; 0,90 — −10%. "
        "Сценарий не меняет сам план, он меняет условия, в которых этот план проверяется."
    )
    cols = st.columns(2)
    ...
    with st.expander("Хранилище ZBO: что меняется"):
        st.caption(
            "Здесь задаётся ухудшение или улучшение уже активного ZBO в пользовательском сценарии. "
            "«Потери × 1,5» означает, что доля потерь становится в 1,5 раза выше расчётной; "
            "«Ёмкость × 0,8» означает доступно 80% расчётной ёмкости. Эти поля не включают ZBO сами по себе."
        )
        cols = st.columns(2)
        storage_loss = cols[0].number_input(
            ...
            step=0.05,
            key="custom-zbo-loss",
            help="Множитель к активной норме потерь ZBO. 1,0 = без изменений.",
        )
        storage_capacity = cols[1].number_input(
            ...
            value=1.0,
            step=0.05,
            key="custom-zbo-capacity",
            help="Множитель к доступной ёмкости ZBO. 1,0 = без изменений.",
        )
