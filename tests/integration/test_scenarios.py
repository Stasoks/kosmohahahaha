...
def test_base_does_not_apply_reliability(base_result):
    ...
    assert january["planned_arrival_by_source"]["A"] == pytest.approx(january["actual_delivered_by_source"]["A"])
...
def test_mandatory_stress_uses_yaml_demand_price_and_delivery(base_result, stress_result):
    ...
    assert march["actual_delivered_by_source"]["D"] == pytest.approx(march["planned_arrival_by_source"]["D"] * 0.55)


def test_stress_share_is_not_multiplied_by_reliability(stress_result):
    march = next(row for row in stress_result.monthly if row["month"] == "2038-03")
    planned = march["planned_arrival_by_source"]["D"]
    assert march["actual_delivered_by_source"]["D"] == pytest.approx(planned * 0.55)
    assert march["actual_delivered_by_source"]["D"] != pytest.approx(planned * 0.55 * 0.78)
